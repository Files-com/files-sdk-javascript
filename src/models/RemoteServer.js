...
class RemoteServer {
  ...
  static listForTesting = async (params = {}, options = {}) => {
    if (params['remote_server_id'] && !isInt(params['remote_server_id'])) {
      throw new Error(`Bad parameter: remote_server_id must be of type Int, received ${getType(remote_server_id)}`)
    }

    if (params['root'] && !isString(params['root'])) {
      throw new Error(`Bad parameter: root must be of type String, received ${getType(root)}`)
    }
    ...
    const response = await Api.sendRequest(`/remote_servers/list_for_testing`, 'GET', params, options)
