...
```
const scheduled_export = await ScheduledExport.find(id)

await scheduled_export.update({
  'name': "Monthly access review",
  'export_type': "permission_audit",
  'export_options': {"group_by":"user"},
  'user_id': 1,
