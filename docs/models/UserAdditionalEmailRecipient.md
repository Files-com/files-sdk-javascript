...
```
await UserAdditionalEmailRecipient.list({
  'user_id': 1,
})
```
...
```
await UserAdditionalEmailRecipient.find(id)
```
...
```
await UserAdditionalEmailRecipient.create({
  'user_id': 1,
  'email': "user-copy@example.com",
})
```
...
```
const user_additional_email_recipient = await UserAdditionalEmailRecipient.find(id)

await user_additional_email_recipient.update({
  'email': "user-copy@example.com",
})
```
...
await user_additional_email_recipient.delete()
```
